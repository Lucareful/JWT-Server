...
import (
	...
	"github.com/luenci/oauth2/pkg/errors"
)
...
func NewServer(cfg *Config, manager Manager) *Server {
	srv := &Server{
		Config:  cfg,
		Manager: manager,
	}
...
func (s *Server) CheckResponseType(rt pkg.ResponseType) bool {
...
func (s *Server) CheckCodeChallengeMethod(ccm pkg.CodeChallengeMethod) bool {
...
func (s *Server) ValidationAuthorizeRequest(r *http.Request) (*AuthorizeRequest, error) {
	...
	resType := pkg.ResponseType(r.FormValue("response_type"))
	if resType.String() == "" {
	...
	ccm := pkg.CodeChallengeMethod(r.FormValue("code_challenge_method"))
	// set default
	if ccm == "" {
		ccm = pkg.CodeChallengePlain
	}
...
func (s *Server) GetAuthorizeToken(ctx context.Context, req *AuthorizeRequest) (models.TokenInfo, error) {
	...
	if fn := s.ClientAuthorizedHandler; fn != nil {
		gt := pkg.AuthorizationCode
		if req.ResponseType == pkg.Token {
			gt = pkg.Implicit
		}
...
func (s *Server) GetAuthorizeData(rt pkg.ResponseType, ti models.TokenInfo) map[string]interface{} {
	if rt == pkg.Code {
...
func (s *Server) ValidationTokenRequest(r *http.Request) (pkg.GrantType, *TokenGenerateRequest, error) {
	if v := r.Method; !(v == "POST" ||
		(s.Config.AllowGetAccessRequest && v == "GET")) {
		return "", nil, errors.ErrInvalidRequest
	}

	gt := pkg.GrantType(r.FormValue("grant_type"))
	if gt.String() == "" {
...
func (s *Server) CheckGrantType(gt pkg.GrantType) bool {
...
func (s *Server) GetAccessToken(ctx context.Context, gt pkg.GrantType, tgr *TokenGenerateRequest) (models.TokenInfo,
	error) {
	...
	case pkg.AuthorizationCode:
		...
		return ti, nil
	case pkg.PasswordCredentials, pkg.ClientCredentials:
		if fn := s.ClientScopeHandler; fn != nil {
			allowed, err := fn(tgr)
			if err != nil {
				return nil, err
